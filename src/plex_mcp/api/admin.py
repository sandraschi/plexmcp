    ...
    try:
        ...
        for user in users:
            ...
            if hasattr(user, 'sections'):
                sections = [section.key for section in user.sections()] if callable(getattr(user, 'sections', None)) else []
            ...
            user_permissions.append(UserPermissions(
...
async def update_user_permissions(
    ...
    try:
        ...
        if hasattr(updated_user, 'sections'):
            sections = [section.key for section in updated_user.sections()] if callable(getattr(updated_user, 'sections', None)) else []
        
        # Get restrictions
        restrictions = []
        if hasattr(updated_user, 'restrictions'):
            restrictions = [str(r) for r in updated_user.restrictions] if hasattr(updated_user.restrictions, '__iter__') else []
