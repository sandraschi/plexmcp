...
class MediaEnrichmentService:
    ...
    async def enrich_media(self, title: str, year: Optional[int] = None, media_type: str = "movie") -> dict[str, Any]:
        """Fetch enrichment data from Wikipedia and other sources."""
        results = {
            "wikipedia": await self.fetch_wikipedia_summary(title, year, media_type),
            "external_sources": []
        }
    ...
    async def fetch_wikipedia_summary(self, title: str, year: Optional[int] = None, media_type: str = "movie") -> Optional[dict[str, Any]]:
