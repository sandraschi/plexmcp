...
class PlexService:
    ...
    async def organize_library(self, library_id: str, dry_run: bool = False, patterns: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    ...
    async def refresh_metadata(self, item_id: Optional[str] = None, library_id: Optional[str] = None, force: bool = False) -> Dict[str, Any]:
    ...
    async def create_user(self, username: str, email: str, password: str, role: str, restricted: bool = False) -> Dict[str, Any]:
    ...
    async def list_libraries(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of all libraries with their information
        """
        if not self._initialized:
            await self.connect()
            
        try:
            sections = await self._run_in_executor(
                lambda: self.server.library.sections()
            )
            return [self._format_library_section(section) for section in sections]
        except Exception as e:
            logger.error(f"Error listing libraries: {e}")
            return []
            
    async def add_library(self, name: str, libtype: str, agent: str, scanner: str, 
                         language: str, location: str, **kwargs) -> Optional[Dict[str, Any]]:
        """Add a new library.
    ...
    async def get_library_items(self, library_id: str, limit: int = 100, offset: int = 0, 
                              sort: Optional[str] = None, **filters) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'items': [self._format_media_item(item) for item in items],
                'total': total,
                'offset': offset,
                'limit': limit
            }
    ...
    async def _format_media_item(self, item) -> Dict[str, Any]:
        ...
        result = {
            ...
            'type': item.type,
            'year': getattr(item, 'year', None),
            'added_at': item.addedAt.timestamp() if hasattr(item, 'addedAt') else 0,
            'updated_at': item.updatedAt.timestamp() if hasattr(item, 'updatedAt') else 0,
            'view_count': getattr(item, 'viewCount', 0),
            'last_viewed_at': item.lastViewedAt.timestamp() if hasattr(item, 'lastViewedAt') else None,
            'thumb': getattr(item, 'thumb', ''),
            ...
            'duration': getattr(item, 'duration', 0) / 60000,  # Convert to minutes
        ...
        if hasattr(item, 'originallyAvailableAt'):
            result['originally_available_at'] = item.originallyAvailableAt.timestamp()
    ...
    async def search_media(
        ...
        **kwargs
        ...
        try:
            ...
            # List filters
            for field in ['genre', 'actor', 'director', 'content_rating', 'studio', 'country', 'language', 'collection']:
                value = locals()[field]
                if value:
