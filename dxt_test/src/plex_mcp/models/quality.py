...
class TranscodingStatus(BaseModel):
    ...
    current_jobs: List[Dict[str, Any]] = Field(
        description="Details of current transcoding jobs"
    )
...
class BandwidthAnalysis(BaseModel):
    ...
    transcoding_overhead: float = Field(
        description="Bandwidth overhead from transcoding"
    )
    client_breakdown: List[Dict[str, Any]] = Field(
        description="Bandwidth usage by client"
    )
    quality_distribution: Dict[str, int] = Field(
        description="Stream quality distribution"
    )
    optimization_suggestions: List[str] = Field(
        description="Bandwidth optimization recommendations"
    )
    cost_estimate: Optional[float] = Field(
        description="Estimated cost if using metered connection"
    )
