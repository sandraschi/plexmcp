...
from plexapi.exceptions import BadRequest, NotFound, Unauthorized, BadConfig
from plexapi.video import Movie, Show, Season, Episode, LibrarySection, Playlist
from plexapi.media import Media, Part, Session
...
from ..models.media import (
    MediaItem, 
    MediaType,
    LibrarySection as LibrarySectionModel,
    MediaFilter,
    SortOrder,
    MediaQuery,
    ...
    TranscodeSession as TranscodeSessionModel,
    Device as DeviceModel,
    ServerPreferences as ServerPreferencesModel,
...
class PlexMediaService(BaseService):
    ...
    async def _initialize(self) -> None:
        ...
        try:
            ...
            raise ServiceError(
                f"Invalid Plex server configuration: {str(e)}",
                code="config_error",
                status_code=400
            ) from e
        ...
        **kwargs
        ...
        try:
            ...
            media = await asyncio.to_thread(
                self._plex.fetchItem, int(media_id)
            )
    ...
    async def _send_playback_command(
        ...
        **params
        ...
        try:
            ...
            raise ServiceError(
                f"Playback command failed: {str(e)}",
    ...
    async def get_playlists(
        ...
        try:
            ...
            if user is not None:
                playlists = [p for p in playlists if str(p.username).lower() == str(user).lower() or 
                           str(p.userID) == str(user)]
            ...
            raise ServiceError(
                f"Failed to get playlists: {str(e)}",
                code="playlist_error"
            ) from e
    ...
    async def get_playlist(
        ...
        try:
            playlist = await asyncio.to_thread(
                self._plex.fetchItem, int(playlist_id)
            )
            
            if not isinstance(playlist, Playlist):
                raise ServiceError(
                    f"Item {playlist_id} is not a playlist",
                    code="not_a_playlist"
                )
            ...
            result = {
                ...
                "duration_str": str(datetime.timedelta(seconds=playlist.duration//1000)) if playlist.duration else None,
    ...
    async def create_playlist(
        ...
        **kwargs
        ...
        try:
            ...
            for item_id in items:
                try:
                    item = await asyncio.to_thread(
                        self._plex.fetchItem, int(item_id)
                    )
            ...
            if not media_items:
                raise ServiceError(
                    "No valid items provided for playlist",
                    code="no_valid_items"
                )
        ...
        try:
            ...
            if title is not None:
                await asyncio.to_thread(
                    setattr, playlist, 'title', title
                )
            
            # Update summary if provided
            if summary is not None:
                await asyncio.to_thread(
                    setattr, playlist, 'summary', summary
                )
    ...
    async def add_to_playlist(
        ...
        try:
            ...
            if not media_items:
                raise ServiceError(
                    "No valid items to add to playlist",
                    code="no_valid_items"
                )
            ...
            raise ServiceError(
                f"Failed to add items to playlist: {str(e)}",
                code="playlist_update_failed"
            ) from e
    ...
    async def remove_from_playlist(
        ...
        try:
            ...
            raise ServiceError(
                f"Failed to remove items from playlist: {str(e)}",
                code="playlist_update_failed"
            ) from e
            ...
            return [
                {
                    ...
                    "last_seen": client.lastSeenAt.isoformat() if hasattr(client, 'lastSeenAt') and client.lastSeenAt else None
                }
            ...
            for session in self._sessions.values():
                try:
                    session_info = {
                        ...
                        } if hasattr(session, 'user') else None,
                        "player": {
                        ...
                        } if hasattr(session, 'player') else None,
                        "media": []
                    }
                            ...
                            for part in media.parts:
                                ...
                                for stream in part.streams:
                                    stream_info = {
                                        ...
                                        "language_code": stream.languageCode if hasattr(stream, 'languageCode') else None,
                                        "selected": stream.selected,
                                        "title": stream.title if hasattr(stream, 'title') else None,
                                        "display_title": stream.displayTitle if hasattr(stream, 'displayTitle') else None
                                    }
                                    ...
                                    if hasattr(stream, 'width'):
                                        stream_info.update({
                                            ...
                                            "color_space": stream.colorSpace if hasattr(stream, 'colorSpace') else None,
                                            "color_range": stream.colorRange if hasattr(stream, 'colorRange') else None,
                                            "color_primaries": stream.colorPrimaries if hasattr(stream, 'colorPrimaries') else None,
                                            "color_trc": stream.colorTrc if hasattr(stream, 'colorTrc') else None,
                                            "ref_frames": stream.refFrames if hasattr(stream, 'refFrames') else None
                                        })
                                    ...
                                    if hasattr(stream, 'audioChannelLayout'):
                                        stream_info.update({
                                            ...
                                            "sampling_rate": stream.samplingRate if hasattr(stream, 'samplingRate') else None,
                                            "bit_depth": stream.bitDepth if hasattr(stream, 'bitDepth') else None,
                                            "bitrate_mode": stream.bitrateMode if hasattr(stream, 'bitrateMode') else None,
                                            "dialog_norm": stream.dialogNorm if hasattr(stream, 'dialogNorm') else None
                                        })
                                    ...
                                    if hasattr(stream, 'subtitleFormat'):
                                        stream_info.update({
                                            ...
                                            "hearing_impaired": stream.hearingImpaired if hasattr(stream, 'hearingImpaired') else False
                                        })
    ...
    @service_method(log_execution=True)
    async def terminate_all_sessions(self, user_id: Optional[Union[str, int]] = None) -> Dict[str, Any]:
        ...
        for session in sessions:
            try:
                result = await self.terminate_session(session["session_key"])
        ...
        try:
            ...
            if section_id is not None:
                library = await asyncio.to_thread(
                    self._plex.library.sectionByID, int(section_id)
                )
            ...
            raise ServiceError(
                f"Failed to get recently added items: {str(e)}",
                code="recently_added_failed"
            ) from e
    ...
    async def get_on_deck(
        ...
        try:
            ...
            raise ServiceError(
                f"Failed to get on deck items: {str(e)}",
                code="on_deck_failed"
            ) from e
    ...
    async def get_recently_played(
        ...
        try:
            ...
            raise ServiceError(
                f"Failed to get recently played items: {str(e)}",
                code="recently_played_failed"
            ) from e
    ...
    async def get_media_metadata(self, item_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ServiceError(
                f"Media item not found: {item_id}",
                code="not_found",
                status_code=404
            )
        except Exception as e:
            ...
            raise ServiceError(
                f"Failed to get media metadata: {str(e)}",
    ...
    def _format_media_item(self, item, include_metadata: bool = False) -> Dict[str, Any]:
        ...
        formatted = {
            ...
            "duration_str": str(datetime.timedelta(seconds=item.duration//1000)) if hasattr(item, 'duration') and item.duration else None,
            ...
            "originally_available_at": item.originallyAvailableAt.isoformat() if hasattr(item, 'originallyAvailableAt') and item.originallyAvailableAt else None,
            ...
            "progress": (item.viewOffset / item.duration * 100) if hasattr(item, 'viewOffset') and hasattr(item, 'duration') and item.duration > 0 else 0
        }
        ...
        if hasattr(item, 'type'):
            if item.type == 'movie':
                formatted.update({
                    ...
                    "chapters": [{"id": ch.id, "title": ch.title, "start": ch.start, "end": ch.end} for ch in item.chapters()] if hasattr(item, 'chapters') else []
                })
            elif item.type == 'show':
                formatted.update({
                    "studio": getattr(item, 'studio', None),
                    ...
                    "seasons": [{"id": s.ratingKey, "title": s.title, "index": s.index, "thumb": s.thumb} for s in item.seasons()] if hasattr(item, 'seasons') and include_metadata else []
                })
                ...
                formatted.update({
                    ...
                    "episode_count": getattr(item, 'leafCount', 0),
                    "unwatched_episodes": getattr(item, 'unwatchedLeafCount', 0),
                    "episodes": [{"id": e.ratingKey, "title": e.title, "index": e.index, "thumb": e.thumb} for e in item.episodes()] if hasattr(item, 'episodes') and include_metadata else []
                })
                ...
                formatted.update({
                    ...
                    "albums": [{"id": a.ratingKey, "title": a.title, "year": a.year, "thumb": a.thumb} for a in item.albums()] if hasattr(item, 'albums') and include_metadata else []
                })
                ...
                formatted.update({
                    ...
                    "track_count": getattr(item, 'leafCount', 0),
                    "genres": [g.tag for g in item.genres] if hasattr(item, 'genres') else [],
                    "tracks": [{"id": t.ratingKey, "title": t.title, "track_number": t.trackNumber} for t in item.tracks()] if hasattr(item, 'tracks') and include_metadata else []
                })
                ...
                formatted.update({
                    ...
                    "media": [self._format_media_part(part) for part in item.media] if hasattr(item, 'media') else []
                })
                ...
                formatted.update({
                    ...
                    "taken_at": item.originallyAvailableAt.isoformat() if hasattr(item, 'originallyAvailableAt') and item.originallyAvailableAt else None,
        ...
        if include_metadata:
            ...
            if hasattr(item, 'actors'):
                try:
                    formatted["actors"] = [{"id": a.id, "tag": a.tag, "role": a.role, "thumb": a.thumb} for a in item.actors]
                except Exception as e:
            ...
            if hasattr(item, 'chapters'):
                try:
                    formatted["chapters"] = [{"id": ch.id, "title": ch.title, "start": ch.start, "end": ch.end} for ch in item.chapters()]
                except Exception as e:
    ...
    async def get_library_sections(self) -> List[LibrarySectionModel]:
        ...
        try:
            ...
            raise ServiceError(
                f"Failed to get library sections: {str(e)}",
                code="library_error"
            ) from e
    ...
    async def search_media(
        ...
        try:
            ...
            return [
                self._plex_item_to_media_item(item)
                for item in results
            ]
    ...
    def _plex_item_to_media_item(self, item) -> MediaItem:
        ...
        if isinstance(item, Movie):
            ...
            media_item.media_type = MediaType.SHOW if isinstance(item, Show) else \
                                  MediaType.SEASON if isinstance(item, Season) else \
                                  MediaType.EPISODE
    ...
    async def refresh_library(self, section_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            if section_id:
                ...
                return {"status": "success", "section_id": section_id, "section_name": section.title}
            else:
