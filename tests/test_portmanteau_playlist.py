...
class TestPlexPlaylist:
    ...
    async def test_list_operation(self, mock_plex_service):
        ...
        with patch("plex_mcp.tools.portmanteau.playlist._get_plex_service", return_value=mock_plex_service):
            result = tool_payload(
                await (plex_playlist.fn if hasattr(plex_playlist, "fn") else plex_playlist)(operation="list")
            )
    ...
    async def test_get_operation(self, mock_plex_service):
        ...
        with patch("plex_mcp.tools.portmanteau.playlist._get_plex_service", return_value=mock_plex_service):
            result = tool_payload(
                await (plex_playlist.fn if hasattr(plex_playlist, "fn") else plex_playlist)(
    ...
    async def test_create_operation_missing_required(self):
        ...
        result = tool_payload(
            await (plex_playlist.fn if hasattr(plex_playlist, "fn") else plex_playlist)(operation="create")
        )
