...
export function RAGDashboard() {
	...
	async function startSync(type: "metadata" | "subtitles" = "metadata") {
		setError(null);
		setLogs([]);
		const msg = type === "metadata" ? "metadata vectorization" : "subtitle dialogue indexing";
		addLog(`[SYSTEM] Initializing ${msg} sequence...`);
		addLog("[CORE] Preparing deep recursive traversal...");
	...
	return (
		<div className="flex flex-col gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
			...
			<div className="grid grid-cols-1 md:grid-cols-5 gap-4">
				<StatCard
					label="Metadata Vectors"
					value={stats?.count?.toLocaleString() || "0"}
					subValue="Title/Summary docs"
					icon={Database}
					color="amber"
				/>
				<StatCard
					label="Dialogue Fragments"
					value={stats?.subtitle_count?.toLocaleString() || "0"}
					subValue="Timestamped snippets"
					icon={FileText}
					color="purple"
				/>
				<StatCard
				...
				<StatCard
					...
					value="Recursive"
					subValue="Deep Indexing"
					icon={Zap}
					color="purple"
				/>
				<StatCard
					label="System Status"
					value={isSyncing ? "BUSY" : "IDLE"}
					subValue={isSyncing ? "Processing RAG task..." : "Standby"}
					icon={Activity}
				...
				<div className="lg:col-span-4 flex flex-col gap-6">
					<div className="glass-panel p-6 flex flex-col gap-6">
						...
						<p className="text-sm text-slate-400 group relative">
							Synchronize your Plex metadata with the vector store. This process recursively indexes
							Movies, Shows (Episodes), and Music (Albums) with parent context enrichment.
							<span className="block mt-2 text-amber/80 text-xs italic">
								SOTA Industrial Standard v13.1 Compliance
							</span>
						</p>
						...
						>
							...
							{isSyncing ? "INGESTION IN PROGRESS" : "SYNC METADATA"}
						</button>

						<button
							onClick={() => startSync("subtitles")}
							disabled={isSyncing}
							className={`w-full py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${
								isSyncing
									? "bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700"
									: "bg-purple-600 text-white hover:bg-purple-500 active:scale-[0.98] shadow-lg shadow-purple-900/20"
							}`}
						>
							{isSyncing ? (
								<Activity className="w-5 h-5 animate-pulse" />
							...
							{isSyncing ? "INGESTION IN PROGRESS" : "SYNC SUBTITLES"}
						</button>
								...
								<div className="flex items-center gap-4">
									...
									<div className="w-24 h-1.5 bg-slate-800 rounded-full overflow-hidden">
										<div
											...
											style={{
												width: `${((syncStatus?.library_index || 0) / (syncStatus?.libraries_total || 1)) * 100}%`,
											}}
										/>
									</div>
								</div>
...
function StatCard({ label, value, subValue, icon: Icon, color, active }: any) {
	...
	return (
		<div
			className={`glass-panel p-4 border-b-2 transition-all duration-500 ${active ? "border-amber translate-y-[-2px]" : "border-transparent"}`}
		>
