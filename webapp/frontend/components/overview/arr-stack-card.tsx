...
import type { ArrStackResponse, ArrServiceStatus } from "@/utils/api";
    ...
    return (
      <div className="rounded-xl glass-panel border border-slate-600/50 p-4">
        ...
        <p className="text-sm text-slate-500 mb-2">
          Optional: if your *arr apps run in Docker (media stack), add each service&apos;s base URL and API key in
          Settings. Same snapshot is available to the MCP <code className="text-amber text-xs">arr_stack</code> tool.
        </p>
  ...
  return (
    <div className="rounded-xl glass-panel border border-slate-600/50 p-4">
      ...
      <p className="text-xs text-slate-600 mt-3">
        Read-only HTTP checks from the webapp backend to your stack (Docker host, LAN, or proxy URL). Use each app&apos;s
        UI to manage downloads.
      </p>
    </div>
  );
}
