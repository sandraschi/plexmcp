...
function formatScalar(key: string, value: unknown): string {
  ...
  if (typeof value === "number") {
    ...
    if (kl.includes("timestamp") || kl === "updated_at" || kl === "created_at" || kl === "added_at") {
...
function LibrariesTable({ rows }: { rows: Record<string, unknown>[] }) {
  ...
  const preferred = ["title", "name", "type", "key", "id", "uuid", "count", "agent", "scanner", "language"];
  const keys = new Set<string>();
  rows.forEach((r) => Object.keys(r).forEach((k) => keys.add(k)));
  const columns = [...preferred.filter((k) => keys.has(k)), ...[...keys].filter((k) => !preferred.includes(k))];
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-t border-slate-700/50 hover:bg-slate-800/40">
              {columns.map((c) => (
                <td key={c} className="px-3 py-2 text-slate-200 whitespace-nowrap max-w-[14rem] truncate">
    ...
    <div className="space-y-8">
      ...
      {!isPlainObject(status) && libraries === undefined && (
        <p className="text-slate-500 text-sm">
          No server status block in this response. Switch to <strong className="text-slate-300">Raw JSON</strong> to
          inspect.
        </p>
      )}
    </div>
