    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="glass-panel p-4 flex flex-col gap-3 group relative">
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 flex gap-2">
            <button onClick={() => setAudioOffset("-0.5")} className="px-2 py-1 bg-slate-800 rounded text-[10px] hover:bg-slate-700 transition-colors">-0.5s</button>
            <input 
              type="text" 
              value={audioOffset} 
          ...
          <button 
            disabled={pending}
            onClick={() => execute("sync_audio", { offset_seconds: parseFloat(audioOffset) })}
        ...
        <p className="text-[10px] text-slate-500 mt-1 italic">Positive for delayed audio, negative for early audio.</p>
      </div>
      ...
      <div className="glass-panel p-4 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            disabled={pending}
            onClick={() => execute("sync_subtitles", { offset_seconds: parseFloat(subOffset) })}
      ...
      <div className="glass-panel p-4 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 mt-1">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={reencode} onChange={e => setReencode(e.target.checked)} className="rounded bg-slate-800 border-slate-700 text-amber" />
        ...
        <div className="glass-panel p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 max-h-[150px] overflow-y-auto pr-1">
            {subStreams.map((s: any) => (
              <div key={s.index} className="flex items-center justify-between p-2 rounded bg-slate-900/50 border border-slate-800/50">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-300">Stream #{s.index} ({s.codec_name})</span>
                  <span className="text-[9px] text-slate-500 uppercase font-bold">{s.tags?.language || "und"}</span>
                </div>
                <button 
                  disabled={pending}
