...
export function RepairDashboard() {
  ...
  async function handleProbe(item: any) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/repair/probe?media_key=${item.key}`, { method: "POST" });
      const data = await res.json();
      if (data.success) {
          ...
          <div className="glass-panel p-4 flex flex-col gap-4">
            ...
            {!selectedLibrary ? (
              ...
              <div className="flex flex-col gap-4">
                <button 
                  onClick={() => { setSelectedLibrary(null); setItems([]); }}
                ...
                <div className="max-h-[500px] overflow-y-auto space-y-2 pr-2">
                  {loading && items.length === 0 ? (
                    <div className="text-slate-500 text-sm animate-pulse p-4">Indexing media...</div>
                  ) : (
                    items.map(item => (
                      <button
                        key={item.key}
              ...
              <div className="flex flex-col items-center gap-4 max-w-sm">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-2">
                    Select a media item from your library browser to initialize the repair workstation and technical probe.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1 flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber text-slate-900 uppercase">Active Workpiece</span>
                      {selectedItem.year && <span className="text-slate-500 text-xs">{selectedItem.year}</span>}
                    </div>
