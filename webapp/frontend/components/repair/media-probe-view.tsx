...
import { Info, Activity, Box, Music, Type, Languages } from "lucide-react";
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {stream.tags?.language && (
                    <span className="text-[10px] text-slate-500 uppercase font-bold">{stream.tags.language}</span>
                  )}
                </div>
                {stream.tags?.title && (
                  <span className="text-[10px] text-slate-600 truncate max-w-[120px]">{stream.tags.title}</span>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-500">
                {isVideo && (
                  <>
                    <div>Res: {stream.width}x{stream.height}</div>
                    <div>FPS: {eval(stream.r_frame_rate || "0").toFixed(2)}</div>
                ...
                {isAudio && (
                  <>
                    <div>Ch: {stream.channels} ({stream.channel_layout})</div>
