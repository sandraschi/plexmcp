...
import { Film, HelpCircle, FileText, ChevronDown, ExternalLink, Container } from "lucide-react";
...
export function Topbar() {
  ...
  const handleWebappClick = async (app: { label: string; url: string; port?: number }) => {
    ...
    try {
      ...
      if (!r.ok) {
        setLaunchModal((m) =>
          m ? { ...m, status: "error", error: data.detail ?? data.error ?? `HTTP ${r.status}` } : null
        );
        return;
      }
      if (data.error) {
      ...
      setLaunchModal((m) =>
        m ? { ...m, status: "error", error: e instanceof Error ? e.message : "Request failed" } : null
      );
    }
  };
