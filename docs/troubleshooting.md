...
+
+## Startup issues (Antigravity / Stdio mode)
+
+- **AttributeError: 'NullLogger' object has no attribute 'handlers'**: This occurs in `FastMCP 3.1+` when it attempts to configure logging in a non-TTY environment. Ensure you are running version **v2.3.1+** of PlexMCP, which includes the hardened `NullLogger` compatible with the latest FastMCP requirements.
